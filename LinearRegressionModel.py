...
print(f"Multiple Regression Model Intercept: {multRegMod.intercept_[0]}")
print(f"Multiple Regression Model Weight: {multRegMod.coef_[0]}")
...
#Load the dataset
housing_all = pd.read_csv('data.csv')

#Keep subset of features, and drop missing values
housing = housing_all[['price','bedrooms','sqft_living','sqft_lot','floors','view','condition','sqft_above','sqft_basement','yr_built','yr_renovated','street','city']]

#Define input and output features
x = housing[['sqft_living', 'bedrooms']].values.reshape(-1, 2)
y = housing[['price']].values.reshape(-1, 1)
...
print("-------------------------house-prices.csv dataset--------------------------")
#Load the dataset
